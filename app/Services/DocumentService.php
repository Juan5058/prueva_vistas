...
class DocumentService
{
    ...
    public function create(array $payload, ?UploadedFile $file = null): ArchiveDocument
    {
        if (($payload['support'] ?? 'Electrónico') === 'Electrónico' && ! $file) {
        ...
        if ($file) {
            ...
            $filePath = $file->storeAs('documents', $uuid.'.pdf');
            $metadata = [
    ...
    public function view(string $id): ArchiveDocument
    {
    ...
    private function audit(ArchiveDocument $document, string $action): void
    {
        $user = request()->user();
